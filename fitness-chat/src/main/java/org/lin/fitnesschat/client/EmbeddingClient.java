...
public class EmbeddingClient {
    ...
    public List<float[]> embed(List<String> texts) {
        try {
            ...
            // 不抛出异常，返回空列表，让上层服务降级到纯文本搜索
            logger.warn("向量生成失败，将使用纯文本搜索作为降级方案");
    ...
    private String callApiOnce(List<String> batch) {
        ...
        return webClient.post()
                ...
                .retryWhen(Retry.backoff(3, Duration.ofSeconds(2)) // 指数退避重试：2s, 4s, 8s
                        .maxBackoff(Duration.ofSeconds(30)) // 最大退避时间 30s
                        .filter(this::isRetryableError) // 只重试可恢复的错误
                        .onRetryExhaustedThrow((spec, signal) -> {
                            logger.error("向量 API 重试次数耗尽，最后一次错误: {}", signal.failure().getMessage());
                            return signal.failure();
                        }))
                .block(Duration.ofSeconds(60)); // 增加超时时间到 60 秒
    }
    ...
    private boolean isRetryableError(Throwable error) {
        ...
        if (error instanceof SocketException || 
            ...
            logger.warn("检测到网络错误，准备重试: {}", error.getMessage());
            return true;
        }
        ...
        if (error instanceof WebClientResponseException) {
            ...
            if (statusCode >= 500 && statusCode < 600) {
                logger.warn("检测到服务器错误 ({}), 准备重试", statusCode);
                return true;
            }
            // 429 Too Many Requests 也可以重试
            if (statusCode == 429) {
                logger.warn("检测到速率限制，准备重试");
                return true;
            }
        }
