...
import org.lin.fitnesschat.entity.DocumentVector;
...
public class ParseService {
    ...
    public void parseAndSave(String fileMd5, InputStream fileStream,
                             String userId, boolean isPublic) throws IOException, TikaException {
        logger.info("开始流式解析文件，fileMd5: {}, userId: {}, orgTag: {}, isPublic: {}",
                fileMd5, userId,isPublic);

        checkMemoryThreshold();

        try (BufferedInputStream bufferedStream = new BufferedInputStream(fileStream, bufferSize)) {
            // 创建一个流式处理器，它会在内部处理父块的切分和子块的保存
            StreamingContentHandler handler = new StreamingContentHandler(fileMd5, userId, isPublic);
    ...
    public void parseAndSave(String fileMd5, InputStream fileStream) throws IOException, TikaException {
        // 使用默认值调用新方法
        parseAndSave(fileMd5, fileStream, "unknown", false);
    }
    ...
    private class StreamingContentHandler extends BodyContentHandler {
        ...
        public StreamingContentHandler(String fileMd5, String userId, boolean isPublic) {
        ...
        private void processParentChunk() {
            ...
            // 2. 将子切片批量保存到数据库
            this.savedChunkCount = ParseService.this.saveChildChunks(fileMd5, childChunks, userId, isPublic, this.savedChunkCount);
    ...
    private int saveChildChunks(String fileMd5, List<String> chunks,
                                String userId, boolean isPublic, int startingChunkId) {
